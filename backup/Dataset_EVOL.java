...
///////////////////////////////
     ...
     */
    static String strU,  strV,  strOmega,  strHBL,  strTp,  strSal,  strTime,  strZeta;
    /**
     * Name of the Variable in NetCDF file
     */
    ...
    private static boolean FLAG_OMEGA;
    /**
    ...
    public static String directory_roms,  sufixe;
    static double[][] pm,  pn;
    String strThetaS, strThetaB, strHc, strPn, strPm;
    static int yearlist100[];
    static String filtre_roms_output;
    static double Topp,  Sopp,  bathy_opp,  Th_opp, DensOpp;
    ...
    static int prof_talu,  SIMU;
    public static int bathy_max;
    static String region, main_output_directory;
...
// Construire le nom des fichier roms, avec l'année et le mois: 
 ...
 /**
    ...
    public void init() throws IOException {
        ...
        System.out.println("number of records per month : " + nbTimeRecords + " (updated each month)");

        FLAG_OMEGA = false;
        if (ncIn.findVariable(strOmega) == null) {
            throw new IOException(
                    "Vertical momentum field " +
                    strOmega + " not found in file " + ncIn.getLocation());
        } else {
            FLAG_OMEGA = true;
        }
        ...
        FLAG_PLANKTON = false;
          if (ncIn.findTopVariable(strMicroZoo) == null
        || ncIn.findTopVariable(strNanoPhyto) == null
        || ncIn.findTopVariable(strDiatoms) == null) {
        throw new IOException(
        "d2 Cannot implement growth limitation. Plankton fields"
...
//        time_tp1 = t0;
    ...
    private void get_dt_HyMo() {
        try {
            ...
            time_tp1 = xTimeTp1.getFloat(xTimeTp1.getIndex().set(0));
            double time_tp0 = xTimeTp1.getFloat(xTimeTp1.getIndex().set(1));

            time_tp1 -= time_tp1 % 60;
            time_tp0 -= time_tp0 % 60;

            dt_HyMo = (int) (Math.abs(time_tp1 - time_tp0)) / 3600 / 24;
    ...
    static void setAllFieldsTp1AtTime(int rank) throws IOException {
        ...
        try {
            ...
            w_tp1 = (float[][][]) ncIn.findVariable(strOmega).read(origin,
                    new int[]{1, nz, (ny - 1), nx}).reduce().copyToNDJavaArray();
            ...
            if (FLAG_PLANKTON) {
                ...
                Diatoms_tp1 = (float[][][]) ncIn.findVariable(strDiatoms).
                        ...
                        copyToNDJavaArray();
                NanoPhyto_tp1 = (float[][][]) ncIn.findVariable(strNanoPhyto).
                        read(origin, new int[]{1, nz, ny, nx}).reduce().
                        copyToNDJavaArray();
                MicroZoo_tp1 = (float[][][]) ncIn.findVariable(strMicroZoo).
                        read(origin, new int[]{1, nz, ny, nx}).reduce().
                        copyToNDJavaArray();
                MesoZoo_tp1 = (float[][][]) ncIn.findVariable(strMesoZoo).
                        read(origin, new int[]{1, nz, ny, nx}).reduce().
                        copyToNDJavaArray();
                O2_tp1 = (float[][][]) ncIn.findVariable(strO2).
                        read(origin, new int[]{1, nz, ny, nx}).reduce().
                        copyToNDJavaArray();
            }
        } catch (IOException e) {
...
// modif TIM: 
    ...
    // Timothee : interpolate u and v
    public static double[] get_uvw(double[] pGrid, double frac) throws
            ArrayIndexOutOfBoundsException {


        double co, CO, x1, x2, x3, u, v, w;
        int n = isCloseToCost(pGrid) ? 1 : 2;
                ...
                for (int ii = 0; ii < n; ii++) {
                    {
                        ...
                        x3 = 0.d;
                        try {
                            x1 = (1.d - frac) * u_tp0[k + kk][j + jj][i + ii] +
                                    frac * u_tp1[k + kk][j + jj][i + ii];
                            x2 = (1.d - frac) * v_tp0[k + kk][j + jj][i + ii] +
                                    frac * v_tp1[k + kk][j + jj][i + ii];
                            x3 = (1.d - frac) * w_tp0[k + kk][j + jj][i + ii] +
                                    frac * w_tp1[k + kk][j + jj][i + ii];
     ...
     * the domain.
     */
    public static double[] getPlankton(double[] pGrid, double frac) {
            ...
            for (int jj = 0; jj < 2; jj++) {
                for (int ii = 0; ii < 2; ii++) {
                    if (isInWater(i + ii, j + jj)) {
                        ...
                        CO += co;
                        x = 0.d;
                        x = (1.d - frac) * Diatoms_tp0[k + kk][j + jj][i + ii] + frac * Diatoms_tp1[k + kk][j + jj][i + ii];
                        Diatoms += x * co;
                        
                        x = (1.d - frac) * NanoPhyto_tp0[k + kk][j + jj][i + ii] + frac * NanoPhyto_tp1[k + kk][j + jj][i + ii];
                        NanoPhyto += x * co;
                        
                        x = (1.d - frac) * MicroZoo_tp0[k + kk][j + jj][i + ii] + frac * MicroZoo_tp1[k + kk][j + jj][i + ii];
                        MicroZoo += x * co;

                        x = (1.d - frac) * MesoZoo_tp0[k + kk][j + jj][i + ii] + frac * MesoZoo_tp1[k + kk][j + jj][i + ii];
                        MesoZoo += x * co;

                        x = (1.d - frac) * O2_tp0[k + kk][j + jj][i + ii] + frac * O2_tp1[k + kk][j + jj][i + ii];
                        O2 += x * co;
                    }
                }
            }
...
//////////
    ...
    public static void load_data(int jour, int year) throws IOException {
        ...
        String fileName = get_filename(year, month);
        open(fileName);
        ...
        // (l'interpolation temporelle ce fait entre Tp0 et Tp1, donc le jour doit être entre les deux rangs)
        rank += 1;
        if (rank > nbTimeRecords - 1) { // {//nbTimeRecords - 1) {
    ...
    public static double[] getFields_uvw(double xRho, double yRho, double zRho, double frac) throws IOException {

        double[] pGrid = {xRho, yRho, zRho};
        // pour la mettre dans la partie advection: 
        double[] uvw;
        uvw = get_uvw(pGrid, frac);
        return (uvw);
    }
    //---------- End of class
}
